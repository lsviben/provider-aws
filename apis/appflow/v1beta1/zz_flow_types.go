...
type DestinationFlowConfigParameters struct {
	...
	// This stores the information that is required to query a particular connector. See Destination Connector Properties for more information.
	DestinationConnectorProperties []DestinationConnectorPropertiesParameters `json:"destinationConnectorProperties,omitempty" tf:"destination_connector_properties,omitempty"`
}
...
type SourceFlowConfigParameters struct {
	...
	// Information that is required to query a particular source connector. See Source Connector Properties for details.
	SourceConnectorProperties []SourceConnectorPropertiesParameters `json:"sourceConnectorProperties,omitempty" tf:"source_connector_properties,omitempty"`
}
...
type UpsolverParameters struct {
	...
	// Configuration that determines how Amazon AppFlow should format the flow output data when Amazon S3 is used as the destination. See S3 Output Format Config for more details.
	S3OutputFormatConfig []UpsolverS3OutputFormatConfigParameters `json:"s3OutputFormatConfig,omitempty" tf:"s3_output_format_config,omitempty"`
}
...
type UpsolverS3OutputFormatConfigParameters struct {
	...
	// Determines the prefix that Amazon AppFlow applies to the folder name in the Amazon S3 bucket. You can name folders according to the flow frequency and date. See Prefix Config for more details.
	PrefixConfig []S3OutputFormatConfigPrefixConfigParameters `json:"prefixConfig,omitempty" tf:"prefix_config,omitempty"`
}
...
type Flow struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.destinationFlowConfig) || has(self.initProvider.destinationFlowConfig)",message="%!s(MISSING) is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.sourceFlowConfig) || has(self.initProvider.sourceFlowConfig)",message="%!s(MISSING) is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.task) || has(self.initProvider.task)",message="%!s(MISSING) is a required parameter"
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.triggerConfig) || has(self.initProvider.triggerConfig)",message="%!s(MISSING) is a required parameter"
