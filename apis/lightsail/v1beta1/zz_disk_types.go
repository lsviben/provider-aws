...
type Disk struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.sizeInGb) || has(self.initProvider.sizeInGb)",message="%!s(MISSING) is a required parameter"
