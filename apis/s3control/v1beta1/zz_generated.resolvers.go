...
func (mg *MultiRegionAccessPoint) ResolveReferences(ctx context.Context, c client.Reader) error {
	...
	for i3 := 0; i3 < len(mg.Spec.InitProvider.Details); i3++ {
		for i4 := 0; i4 < len(mg.Spec.InitProvider.Details[i3].Region); i4++ {
			rsp, err = r.Resolve(ctx, reference.ResolutionRequest{
				CurrentValue: reference.FromPtrValue(mg.Spec.InitProvider.Details[i3].Region[i4].Bucket),
				Extract:      resource.ExtractResourceID(),
				Reference:    mg.Spec.InitProvider.Details[i3].Region[i4].BucketRef,
				Selector:     mg.Spec.InitProvider.Details[i3].Region[i4].BucketSelector,
				To: reference.To{
					List:    &v1beta1.BucketList{},
					Managed: &v1beta1.Bucket{},
				},
			...
			if err != nil {
				return errors.Wrap(err, "mg.Spec.InitProvider.Details[i3].Region[i4].Bucket")
			}
			mg.Spec.InitProvider.Details[i3].Region[i4].Bucket = reference.ToPtrValue(rsp.ResolvedValue)
			mg.Spec.InitProvider.Details[i3].Region[i4].BucketRef = rsp.ResolvedReference
