...
type Fleet struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.computeCapacity) || has(self.initProvider.computeCapacity)",message="%!s(MISSING) is a required parameter"
