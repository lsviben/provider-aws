...
type Budget struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.budgetType) || has(self.initProvider.budgetType)",message="%!s(MISSING) is a required parameter"
