...
func (mg *ResourceDataSync) ResolveReferences(ctx context.Context, c client.Reader) error {
	...
	for i3 := 0; i3 < len(mg.Spec.InitProvider.S3Destination); i3++ {
		rsp, err = r.Resolve(ctx, reference.ResolutionRequest{
			CurrentValue: reference.FromPtrValue(mg.Spec.InitProvider.S3Destination[i3].Region),
			Extract:      resource.ExtractParamPath("region", false),
			Reference:    mg.Spec.InitProvider.S3Destination[i3].RegionRef,
			Selector:     mg.Spec.InitProvider.S3Destination[i3].RegionSelector,
			To: reference.To{
				List:    &v1beta13.BucketList{},
				Managed: &v1beta13.Bucket{},
			},
		...
		if err != nil {
			return errors.Wrap(err, "mg.Spec.InitProvider.S3Destination[i3].Region")
		}
		mg.Spec.InitProvider.S3Destination[i3].Region = reference.ToPtrValue(rsp.ResolvedValue)
		mg.Spec.InitProvider.S3Destination[i3].RegionRef = rsp.ResolvedReference
