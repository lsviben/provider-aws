...
type VPCIpam struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.operatingRegions) || has(self.initProvider.operatingRegions)",message="%!s(MISSING) is a required parameter"
