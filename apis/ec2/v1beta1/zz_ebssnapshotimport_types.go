...
type EBSSnapshotImport struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.diskContainer) || has(self.initProvider.diskContainer)",message="%!s(MISSING) is a required parameter"
