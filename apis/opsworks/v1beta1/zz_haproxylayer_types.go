...
type HAProxyLayer struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.statsPassword) || has(self.initProvider.statsPassword)",message="%!s(MISSING) is a required parameter"
