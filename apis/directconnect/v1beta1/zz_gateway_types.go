...
type Gateway struct {
	...
	// +kubebuilder:validation:XValidation:rule="!('*' in self.managementPolicies || 'Create' in self.managementPolicies || 'Update' in self.managementPolicies) || has(self.forProvider.amazonSideAsn) || has(self.initProvider.amazonSideAsn)",message="%!s(MISSING) is a required parameter"
